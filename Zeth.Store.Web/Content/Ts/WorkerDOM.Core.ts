namespace WorkerDOM {
    ...
    class WViewConfig extends WBase {
        ...
        public getTemplate(): string | WResult<string> {
            if (isNull(this.template) && isNotNull(this.prms.get('template'))) this.template = this.prms.get('template');
        ...
        public getServices(view: WView): WDictionary<any> {
            var i: number = null;
            var c: number = null;
            var services: WDictionary<any> = new WDictionary<any>();

            c = this.services.length;
            for (i = 0; i < c; i++) services.set(this.services[i].name, this.services[i].service(view));
        ...
        constructor(source: IViewConfiguration) {
            ...
            if (isNull(this.template = prms.get('template'))) {
                if (prms.get('templateUrl')) this.template = function () { return httpGet(prms.get('templateUrl')); }
                else {
    ...
    const HTTP_SEND_LOG: IHttpLog = { time: new Date() };

    function httpParseResult(xhr: XMLHttpRequest) {
        if (xhr.getResponseHeader('Content-Type').indexOf('application/json') >= 0) {
            return JSON.parse(xhr.response);
        }
        else return xhr.response;
    }
    function httpSend(method: string, url: string, data?: any, headers?: WDictionary<string>): WResult<any> {
        var result: WResult<string> = new WResult<string>(function (resolve: Function, reject: Function) {
            ...
            xhr.onreadystatechange = function (e) {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) resolve.call(result, httpParseResult(xhr));
                    else reject.call(result, new Error('HTTP Request Error: ' + xhr.response));
                }
            }

            xhr.onerror = function (e) {
                reject.call(result, new Error('HTTP Request Error: can not connect to the URL.'));
            }

            HTTP_SEND_LOG.time = new Date();
            xhr.send(data);
        });

        return result;
    }
    function httpGet(url: string, data?: any, headers?: any): WResult<any> {
        return httpSend('GET', url, data, headers);
    }
    function httpPost(url: string, data?: any, headers?: any): WResult<any> {
        return httpSend('POST', url, data, headers);
    }
    function httpHasError(data: any): boolean {
        return data != null && data.HasError == true || data.ErrorMessage;
    }
    function httpGetError(data: any): string {
        var msg = 'Error';
    ...
    function serviceHttp(view: WView): IHttpService {
        return { sendLog: HTTP_SEND_LOG, hasError: httpHasError, getError: httpGetError, send: httpSend, get: httpGet, post: httpPost };
    }
    //#endregion
    ...
    //#region MessageBox
    const MESSAGE_BOX_TEMPLATE: string = '<div class="content sm"><div class="title" w-bind="Title"></div><div class="body" w-bind="Message"></div><div class="footer"><button type="button" class="btn btn-sm" w-on-click="Ok">Aceptar</button><button type="button" class="btn btn-sm" w-on-click="Cancel" w-if="HasCancel" style="margin-left:5px;">Cancelar</button></div></div>';
    ...
    function init() {
        ...
        //#region Datepicker
        addController({ name: 'Datepicker', controller: datepickerController, params: { autoCreate: false } });
